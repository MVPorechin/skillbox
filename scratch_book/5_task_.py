...
def increase_by_ten(step: int = 1, length: int = 1) -> int:
    """
    ...
    if length == 1:
        ...
        length -= 1
        return increase_by_ten(step=step * 10 + 1, length=length)
...
def simple_or_not(left: int, right: int) -> int:
    ...
    if len(str(right)) - len(str(left)) > 99:
        increase_first_step = increase_by_ten(length=len(str(left)))
        increase_second_step = increase_by_ten(length=len(str(right)))
        ...
        return count(start=left, end=right, step=increase_by_ten(length=len(str(left))))
