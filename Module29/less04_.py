...
def log_methods(format: str):
    def decorator_log_method(decorator: Callable) -> Callable:
        @functools.wraps(decorator)
        def decorate(cls):
            start = time.time()
            # instance = cls(*args, **kwargs)
            print("Время создания -", datetime.datetime.today().strftime(format_convert))
            print("Методы:", end=" ")
            for i_method_name in dir(cls):
                if i_method_name.startswith('__') is False:
                    cur_method = getattr(cls, i_method_name)
                    decorated_method = cls(cur_method)
                    setattr(cls, i_method_name, decorated_method)
            end = time.time()
            run_time = round(end - start, 3)
            print(f'Завершение {cls.__name__}, время работы = {run_time}')
            return cls
        return decorate
